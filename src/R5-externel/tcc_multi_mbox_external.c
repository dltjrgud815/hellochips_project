		...
		const struct tcc_mbox_data *msg)
{
	...
	printk(KERN_INFO"tcc_mbox_send_messgage() func running!!\n");
	return ret;
}
			...
			const struct tcc_mbox_data *msg)
{
	...
	printk(KERN_INFO"tcc_multich_mbox_send_worker() func running!!\n");
}
...
static int32_t tcc_multich_mbox_submit(struct mbox_chan *chan, void *mbox_msg)
{
	...
	printk(KERN_INFO"tcc_multich_mbox_submit() func running!!\n");
	return 0;
}
	...
	if ((dev_id != NULL) && (msg != NULL)) {
		...
		if (chan->cl != NULL) {
			...
			if (msg->cmd[1] > 1) {
				...
				front_or_back = (int)(uint8_t)(msg->data[1]);
				if(front_or_back < 5)
				{
					// file open
				...
				{
				// file open
				file = filp_open("/tmp/front_data.log", O_WRONLY | O_CREAT | O_TRUNC, 0644);
				if (IS_ERR(file)) {
					printk(KERN_ERR "Failed to open file: /tmp/mbox_data.log\n");
					kfree(buffer);
					return;
				...
				snprintf(buffer, 512, "%d %d",
							(int)(uint8_t)msg->data[1], (int)(uint8_t)msg->data[2]);
...
static int32_t tcc_multich_mbox_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "===HELLOCHIPS_MULTIMAILBOX===PROBE");
	return ret;
}
